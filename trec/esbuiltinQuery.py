...
PICKLE_DOCLENGTH_DICT_FILE_LOCATION = "./docLength.pickle"
...
def _get_term_details(term_to_search, query_docs):
    ...
    res = es.search(index=INDEX_NAME, body=query_body, scroll='5m')
    ...
    for j in range(0, 3):
        ...
        for i in range(0, size):
            doc_id = res["hits"]["hits"][i]["_source"]["docno"]
        ...
        sc_result = es.scroll(scroll_id=scroll_id, scroll='5m')

    term_detail = {'df': total_df, 'ttf': ttf, 'matching_docs': matching_docs}
    if term_to_search in ['corrupt']:
    ...
    with open(file_location, mode='w') as result_file:
        for query in ranked_docs:  # range(50, 101):  # accessing all queries in ranked docs
            ...
            for doc in ranked_doclist:
                ...
                result_file.write(
                    query + " prajakta " + doc + " " + str(rank) + " " + score + "\n")
                rank = rank + 1
    result_file.close()
...
def get_length_all_docs():
    ...
    for j in range(0, 1100):
        ...
        if count % 1000 == 0:
            ...
            file = open(PICKLE_DOCLENGTH_DICT_FILE_LOCATION, 'wb')
...
if __name__ == '__main__':
    total_docs = 109215
    print("Total docs, vocab " + str(total_docs))
    # total_length = get_length_all_docs()
